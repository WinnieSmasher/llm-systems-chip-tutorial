...
- RAG 的 indexing 和 retrieval/generation 为什么要拆开？
- top-k 里没有正确证据时，生成模型该不该背锅？
- Agent tool call 为什么必须有权限边界？

课后产出：

- 用 `examples/chunk_text_preview.py` 生成一次 chunk preview。
- 写一份 RAG failure table，至少记录 5 个失败样例。
- 写一份 tool boundary checklist。

自检标准：

- 能解释 loader、chunking、embedding、rerank、generation 各自会在哪里出错。
- 能说明为什么检索到的网页、PDF、README、issue 评论不能当成用户指令。
...
本仓库的 `chunk_text_preview.py` 是教学脚本，主要用于观察 chunk 大小、overlap 和基本 metadata。正式 RAG 管线还要补 PDF 页码、Markdown section、代码符号、权限标签和版本号。
